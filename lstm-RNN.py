...
# using LSTM RNNs. You'll be using part of the Simpsons 
...
# creating a save directory for the LSTM RNN model
save_dir_lstm = './lstm_save'
...
# 3. Create a single RNN cell (with LSTM cells)
...
with train_graph.as_default():
    ...
    with tf.name_scope("lstm_cell"):
        ...
        ############################################################################  
        # You created an RNN Cell above. Time to use the cell to create an RNN.
...
with tf.Session(graph=train_graph) as sess:
    ...
    file_writer = tf.summary.FileWriter('./logs_lstm/1', sess.graph)
    loss = 0
    ...
    saver.save(sess, './logs_lstm/1/embedding.ckpt')
    saver.save(sess, save_dir_lstm)
    print('Model Trained and Saved')
    
# Save parameters for checkpoint
helper.save_params_lstm((seq_length, int_to_vocab, embed_mat, save_dir_lstm))